...
const mediaMessageSHA256B64 = (message) => {
    let media = Object.values(message)[0];
    if (message.groupStatusMessage) {
        media = Object.values(message.groupStatusMessage)[0];
    }
...
function extensionForMediaMessage(message) {
    const getExtension = (mimetype) => mimetype ? mimetype.split(';')[0].split('/')[1] : 'bin';
    let type = Object.keys(message)[0];
    let extension;
    
    if (type === 'groupStatusMessage' && message.groupStatusMessage) {
        type = Object.keys(message.groupStatusMessage).find(k => Defaults_1.MEDIA_KEYS.includes(k)) || type;
    }
    ...
    if (type === 'locationMessage' ||
        ...
        const messageContent = message.groupStatusMessage ? message.groupStatusMessage[type] : message[type];
        extension = messageContent?.mimetype ? getExtension(messageContent.mimetype) : 'bin';
    }
